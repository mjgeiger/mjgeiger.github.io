...
int fillString(char *s, int n) {
	char str[50];		// Input string
	int charsLeft;		// Space left in s --> could just be replaced by n

	do {
		scanf("%s", str);		// Read input string

		charsLeft = n - (strlen(s) + 1);

		if (strlen(str) < charsLeft)
			strcat(s, str);

		else if (strlen(str) == charsLeft) {
			strcat(s, str);
			return strlen(s);
		}
